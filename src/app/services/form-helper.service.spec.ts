...
describe('FormHelperService', () => {
  ...
  describe('getErrorMessage', () => {
    ...
    it('should return \'This is an invalid customer number\' if control value should be a valid customer number but is not', inject([FormHelperService], (service: FormHelperService) => {
      const formControl = new FormControl('', [customerNumberValidator]);
      expect(service.getErrorMessage(formControl)).toBe('This is an invalid customer number');
    }));

    it('should return \'Your input is too long\' if control value should not be that long but is', inject([FormHelperService], (service: FormHelperService) => {
      const formControl = new FormControl('12345678900', [Validators.maxLength(10)]);
      expect(service.getErrorMessage(formControl)).toBe('Your input is too long');
    }));
  });
